...
resource "azurerm_app_service" "my_app_service_container" {
    ...
    app_settings = {
        ...
        MONGO_CONNECTION_STRING = "mongodb://${azurerm_cosmosdb_account.acc.name}:${azurerm_cosmosdb_account.acc.primary_key}@${azurerm_cosmosdb_account.acc.name}.mongo.cosmos.azure.com:10255/?ssl=true&replicaSet=globaldb&retrywrites=false&maxIdleTimeMS=120000&appName=@${azurerm_cosmosdb_account.acc.name}@"
